...
export default function TopBar() {
  ...
  const updatePos = () => {
    ...
    if (!b) return;
    // panel aligned to the bell's right edge
  ...
  const load = async () => {
    try {
      const { orders } = await fetchFromAPI<{ orders: OrderMini[] }>(
        "/dashboardadmin/orders"
      );
      // ensure newest first (backend already sorts, but let’s be safe)
      ...
      const count = sorted.filter(
        (o) => +new Date(o.createdAt) > lastSeen
      ).length;
      setUnseen(count);
    } catch (e) {
      // silent fail; keep previous
  ...
  useLayoutEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      const isInside = (t as HTMLElement).closest("[data-orders-panel]"); // keep open if click inside
