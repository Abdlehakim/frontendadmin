...
/** Normalize to `/api/...` regardless of how caller passes endpoint */
const toApiPath = (endpoint: string): string => {
  const cleaned = endpoint.replace(/^\/+/, ""); // remove leading slashes
...
export async function fetchFromAPI<T = Json>(
  ...
  // Try to parse JSON only if the server says it is JSON
